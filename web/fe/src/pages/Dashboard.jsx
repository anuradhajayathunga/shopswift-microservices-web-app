      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-card rounded-xl border border-border shadow-sm p-6 flex flex-col">
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Actual sales against predictions</p>
            </div>
          ...
          <div className="flex-1 min-h-[300px] w-full bg-muted/20 rounded-lg border border-dashed border-border flex flex-col items-center justify-center relative overflow-hidden">
            ...
            <span className="text-muted-foreground text-sm font-medium">Chart Area Ready</span>
            <span className="text-xs text-muted-foreground/70 mt-1">For Recharts integration</span>
        ...
        <div className="bg-card rounded-xl border border-border shadow-sm p-6 flex flex-col">
          ...
          <div className="space-y-3">
            {[
              { type: '📦', text: 'Order #OD-2456 received', time: '5 mins ago' },
              { type: '✅', text: 'Order #OD-2455 shipped', time: '2 hours ago' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-card rounded-xl border border-border shadow-sm p-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {[
                  { id: '#2456', name: 'John Doe', amount: 'Rs. 12,450', status: 'Processing' },
                  { id: '#2455', name: 'Jane Smith', amount: 'Rs. 8,920', status: 'Shipped' },
                  { id: '#2454', name: 'Mike Johnson', amount: 'Rs. 5,680', status: 'Delivered' },
                  { id: '#2453', name: 'Sarah Lee', amount: 'Rs. 15,320', status: 'Processing' }
                ].map((row, idx) => (
        ...
        <div className="bg-card rounded-xl border border-border shadow-sm p-6">
          <div className="flex justify-between items-center mb-6">
            ...
            <a href="#" className="text-sm text-primary hover:underline">Manage</a>
          </div>
          <div className="space-y-4">
            {[
              { name: 'Winter Jacket', stock: 245, total: 500, color: 'bg-blue-500' },
              { name: 'Casual Shirt', stock: 89, total: 300, color: 'bg-yellow-500' },
              { name: 'Running Shoes', stock: 12, total: 200, color: 'bg-red-500' },
              { name: 'Jeans Blue', stock: 423, total: 450, color: 'bg-green-500' }
            ].map((item, idx) => (
              <div key={idx}>
                <div className="flex justify-between mb-2">
                  <span className="text-sm font-medium">{item.name}</span>
                  <span className="text-xs text-muted-foreground">{item.stock}/{item.total}</span>
                </div>
                <div className="w-full h-2 bg-muted/40 rounded-full overflow-hidden">
                  <div className={`h-full ${item.color}`} style={{ width: `${(item.stock/item.total)*100}%` }} />
                </div>
              </div>
            ))}
          </div>
  ...
  return (
    <div className="bg-card rounded-xl border border-border shadow-sm p-6 hover:shadow-md transition-all">
      <div className="flex justify-between items-start mb-4">
        <div className="h-10 w-10 rounded-lg bg-muted/50 flex items-center justify-center">{icon}</div>
      ...
      <div className="flex items-center gap-2">
        <ArrowUpRight size={16} className={isPositive ? 'text-emerald-500' : 'text-rose-500 rotate-180'} />
